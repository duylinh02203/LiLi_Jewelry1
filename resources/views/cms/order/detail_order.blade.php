        ...
        <div class="row justify-content-center text-center">
            <div class="col-12 col-md-8 col-lg-6 p-4">
                ...
                <div class="d-flex justify-content-between" style="font-size: 14px;">
                    <span>Ngày: {{ date('d/m/Y', strtotime($order->created_at)) }}</span>
                    <span>Mã đơn hàng: DL00{{ $order->id }}</span>
                </div>
                <div class="d-flex justify-content-between mb-3" style="font-size: 14px;">
                    <span>Thu ngân: Hệ thống</span>
                    <span>In lúc: {{ date('H:i:s', strtotime($order->created_at)) }}</span>
                </div>
                <div class="text-start" style="font-size: 15px;">
                    <p><strong>Khách hàng:</strong> {{ $order->name }}</p>
                    <p><strong>SĐT :</strong> {{ $order->phone }}</p>
                    <p><strong>Địa chỉ:</strong> {{ $order->address }}</p>
                    <p><strong>Phương thức thanh toán :</strong> {{ $order->payment == 'cod' ? 'Thanh toán khi nhận hàng (COD)' : ($order->payment == 'vnpay' ? 'Thanh toán online qua VNPAY' : 'Hình thức thanh toán không xác định') }}</p>
                    @php
